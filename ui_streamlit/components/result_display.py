...
def render_kwic_table(kwic_data, is_parallel=False, target_lang="NA", show_meta=True):
    """
    ...
    Args:
        ...
        show_meta (bool): Whether to show the Metadata column.
    """
    if not kwic_data:
    ...
    if show_meta:
        ...
        /* Column Widths (Without Metadata) */
        .collex-table-inner td:nth-child(1) { width: 6%; text-align: left; font-weight: bold; border-right: 1px solid #444; white-space: nowrap; } /* Collocate/Num */
        .collex-table-inner td:nth-child(2) { width: 10%; text-align: center; font-size: 0.8em; white-space: normal; color: #aaa; } /* Source */
        .collex-table-inner td:nth-child(3) { width: 28%; text-align: right; white-space: normal; } /* Left */
        ...
        .collex-table-inner td:nth-child(5) { width: 28%; text-align: left; white-space: normal; } /* Right */
        
        /* Translation Column (if present - child 6) */
        .collex-table-inner td:nth-child(6) { 
            text-align: left; 
            color: #CCFFCC; 
            width: 14%; 
            font-family: sans-serif; 
    ...
    # Reorder columns & headers based on show_meta
    if show_meta:
        ...
        cols = ['Collocate', 'Source Corpus', 'Left Context', 'Node', 'Right Context']
        if is_parallel: cols.append('Translation')
        df = df[cols]
        header = "<tr><th>#</th><th>Source</th><th>Left Context</th><th>Node</th><th>Right Context</th>"
        if is_parallel:
            header += f"<th>Translation ({target_lang})</th>"
        header += "</tr>"
