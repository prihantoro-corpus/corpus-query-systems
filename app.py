...
def generate_kwic(df_corpus, raw_target_input, kwic_left, kwic_right, pattern_collocate_input="", pattern_collocate_pos_input="", pattern_window=0, limit=KWIC_MAX_DISPLAY_LINES, random_sample=False, is_parallel_mode=False):
    ...
    MODIFIED: Now also returns the list of sentence IDs (sent_ids) for parallel lookups.
...
def load_parallel_corpus_file(file_source):
    ...
    for index, row in df_raw.iterrows():
        ...
        # Store the raw translated sentence for quick lookup
        target_sent_map[sent_id_counter] = tgt_text 
        
        # Populate source language data
        ...
        # Populate target language data (only if needed for full corpus analysis later)
        # For now, we only need the map, but having a dataframe is useful for future expansion.
...
def load_corpus_file(file_source, sep=r"\s+"):
    # If a parallel file was loaded, use the result and stop here
    if st.session_state.get('parallel_mode', False) and not st.session_state['df_target_lang'].empty:
         # Use the source language DF generated by the parallel loader
         return pd.DataFrame(st.session_state['df_target_lang'])
    ...
    try:
        ...
        if df_attempt is not None and df_attempt.shape[1] >= 3:
            ...
            # Reset parallel mode if a standard corpus is loaded
            st.session_state['parallel_mode'] = False
            st.session_state['df_target_lang'] = pd.DataFrame()
            st.session_state['target_sent_map'] = {}
    ...
    try:
        ...
        # Reset parallel mode if a raw text file is loaded
        st.session_state['parallel_mode'] = False
        st.session_state['df_target_lang'] = pd.DataFrame()
        st.session_state['target_sent_map'] = {}
...
# -----------------------------------------------------
# MODIFIED: Accepts sent_ids and target_sent_map
...
# -----------------------------------------------------
# COLLOCATION LOGIC (No major change needed, uses KWIC logic for frequency)
# -----------------------------------------------------
@st.cache_data(show_spinner=False)
...
with st.sidebar:
    ...
    # Standard Corpus Upload / Built-in Selection
    if not parallel_uploaded:
        st.markdown("##### 📁 Upload Standard/Built-in Corpus")
