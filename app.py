...
# --------------------------------------------------------
# Zipf and Band Calculation (Cached, as it relies only on input DF)
# --------------------------------------------------------
def pmw_to_zipf(pmw):
    if pmw <= 0: return np.nan
    return math.log10(pmw) + 3

def zipf_to_band(zipf):
    if pd.isna(zipf): return np.nan
    elif zipf >= 7.0: return 1
    elif zipf >= 6.0: return 2
    elif zipf >= 5.0: return 3
    elif zipf >= 4.0: return 4
    else: return 5
# --------------------------------------------------------


# --- Word Cloud Function (CACHED) ---
@st.cache_data
def create_word_cloud(freq_data, is_tagged_mode):
    """Generates a word cloud from frequency data with conditional POS coloring."""
    
    if not WORDCLOUD_FEATURE_AVAILABLE: return None
        
    single_word_freq_data = freq_data[~freq_data['token'].str.contains(' ')].copy()
    if single_word_freq_data.empty: return None 
...
def reset_analysis():
    ...
    st.session_state['corpus_loaded'] = False # CRITICAL: Clear load flag
...
# --- Dictionary Helper: Get all forms by lemma (CACHED) ---
@st.cache_data
def get_all_lemma_forms_details(df_corpus, target_word):
    ...
    if not valid_lemmas: return pd.DataFrame(), [], []
...
# --- KWIC/Concordance Helper Function (CACHED) ---
...
def generate_kwic(df_corpus, raw_target_input, kwic_left, kwic_right, pattern_collocate_input="", pattern_collocate_pos_input="", pattern_window=0, limit=KWIC_MAX_DISPLAY_LINES, random_sample=False, is_parallel_mode=False, show_pos=False, show_lemma=False):
    ...
    if primary_target_len == 1 and not is_structural_search:
        ...
        for i, token in enumerate(tokens_lower):
            if target_pattern.fullmatch(token):
                ...
                matching_tokens_at_node_one.append(df_corpus['token'].iloc[i])
    else:
    ...
    if literal_freq == 0: return ([], 0, raw_target_input, 0, [], pd.DataFrame()) 
    ...
    if is_pattern_search_active and pattern_window > 0:
        ...
        for i in all_target_positions:
            ...
            if found_collocate: final_positions.append(i)
                
    total_matches = len(final_positions)
    if total_matches == 0: return ([], 0, raw_target_input, 0, [], breakdown_df)
    ...
    def format_token_inline(token, pos, lemma, is_collocate_match=False, is_node_word=False):
        token_html = token
        if is_collocate_match: token_html = f"<span style='color: black; background-color: #FFEA00;'>{token}</span>"
        if is_node_word: token_html = f"<b>{token_html}</b>"
        output = [token_html]
        ...
        for k in range(kwic_start, kwic_end):
            ...
            if is_pattern_search_active and not is_node_word:
                 ...
                 if word_matches_highlight and pos_matches_highlight:
                     ...
                     if collocate_to_display == "": collocate_to_display = token 
...
def generate_n_grams(df_corpus, n_size, n_gram_filters, is_raw_mode, corpus_id):
    ...
    def matches_filter(token, token_low, pos_tag, lemma_tag, pattern_str, is_raw_mode):
        if not pattern_str: return True
    ...
    for i in range(total_tokens - n_size + 1):
        ...
        if is_match: matched_n_grams_list.append(tuple(current_tokens))
            
    if not matched_n_grams_list: return pd.DataFrame()
    ...
    def is_only_punc_or_digit(n_gram_str):
        for token in n_gram_str.split():
            if token.lower() not in PUNCTUATION and not token.isdigit(): return False
        return True
...
def extract_xml_structure(file_source, max_values=20):
    ...
    file_to_analyze = file_source[0] if isinstance(file_source, list) else file_source
    ...
    def process_element(element):
        tag = element.tag
        if tag not in structure: structure[tag] = {}
        for attr_name, attr_value in element.attrib.items():
            if attr_name not in structure[tag]: structure[tag][attr_name] = set()
            if len(structure[tag][attr_name]) < max_values: structure[tag][attr_name].add(attr_value)
        for child in element: process_element(child)
...
def format_structure_data_hierarchical(structure_data, indent_level=0, max_values=20):
    ...
    def get_indent(level): return f'<span style="padding-left: {level * 1.5}em;">'
    ...
    for tag in sorted(structure_data.keys()):
        ...
        for attr in sorted(tag_data.keys()):
            ...
            if len(values) > max_values: sampled_values_str += f", ... ({len(values) - max_values} more unique)"
...
def parse_xml_content_to_df(file_source):
    ...
    try:
        ...
        if not lang_code:
            ...
            if lang_match: lang_code = lang_match.group(3) or lang_match.group(2)
    ...
    if not sent_tags: sent_tags = list(root)
    ...
    if not sent_tags:
        ...
        if raw_sentence_text:
            ...
            if tokens:
               for token in tokens: df_data.append({"token": token, "pos": "##", "lemma": "##", "sent_id": 1})
    ...
    for sent_elem in sent_tags:
        ...
        if sent_id_str:
            ...
            except ValueError: sequential_id_counter += 1; sent_id = sequential_id_counter
        else: sequential_id_counter += 1; sent_id = sequential_id_counter

        if sent_id is None: continue
        ...
        if raw_sentence_text: sent_map[sent_id] = raw_sentence_text.strip()
...
def load_monolingual_corpus_files(file_sources, explicit_lang_code, selected_format):
    ...
    if not file_sources: return None
    ...
    if not all_df_data: return None
        
    df_src = pd.DataFrame(all_df_data)
    df_src["_token_low"] = df_src["token"].str.lower()
    
    if xml_detected_lang_code: SOURCE_LANG_CODE = xml_detected_lang_code
...
def load_xml_parallel_corpus(src_file, tgt_file, src_lang_code, tgt_lang_code):
    ...
    if src_sent_ids != tgt_sent_ids:
        ...
        if missing_in_tgt: error_msg += f" Source ({src_result['lang_code']}) is missing sentence IDs: {sorted(list(missing_in_tgt))[:5]}..."
        if missing_in_src: error_msg += f" Target ({tgt_result['lang_code']}) is missing sentence IDs: {sorted(list(missing_in_src))[:5]}..."
        st.error(error_msg)
        return None
...
# ---------------------------------------------------------------------
# EXISTING: Excel Parallel Corpus Loading (NO CACHE)
# ---------------------------------------------------------------------
...
# --- Monolingual File Dispatcher for Built-in (NO CACHE) ---
def load_corpus_file_built_in(raw_content, corpus_name, explicit_lang_code):
    ...
    if is_vertical_format:
        try:
            ...
            df = pd.read_csv(
                file_buffer_for_pandas, sep=r'\s+', header=None, engine="python", dtype=str, skipinitialspace=True,
                usecols=[0, 1, 2], names=['token', 'pos', 'lemma']
            )
    ...
    if df.empty:
        ...
        tokens = [t.strip() for t in cleaned_text.split() if t.strip()] 
        df = pd.DataFrame({
            "token": tokens, "pos": ["##"] * len(tokens), "lemma": ["##"] * len(tokens)
        })
        df['sent_id'] = 0 
    ...
    with st.spinner(f"Generating concordance examples for top {len(colloc_list)} collocates..."):
        for rank, (index, row) in enumerate(colloc_list.iterrows()):
            ...
            kwic_rows, total_matches, _, _, sent_ids, _ = generate_kwic(
                ...
                pattern_collocate_input=collocate_word, pattern_collocate_pos_input="", 
                pattern_window=window, limit=limit_per_collocate, is_parallel_mode=is_parallel_mode, 
                show_pos=show_pos, show_lemma=show_lemma
            )
            ...
            if kwic_rows:
                ...
                collex_rows_total.append({
                    "Collocate": f"{rank+1}. {collocate_word}", "Left Context": kwic_row['Left'],
                    "Node": kwic_row['Node'], "Right Context": kwic_row['Right'],
                    "Translation": translation if is_parallel_mode else None
                })
        
    if collex_rows_total:
        collex_df = pd.DataFrame(collex_rows_total)
        if not is_parallel_mode: collex_df = collex_df.drop(columns=['Translation'])

        header = "<tr><th>Collocate (Rank)</th><th>Left Context</th><th>Node</th><th>Right Context</th>"
        if is_parallel_mode: header += f"<th>Translation ({TARGET_LANG_CODE})</th>"
        header += "</tr>"
...
def generate_collocation_results(df_corpus, raw_target_input, coll_window, mi_min_freq, max_collocates, is_raw_mode, collocate_regex="", collocate_pos_regex_input="", selected_pos_tags=None, collocate_lemma=""):
    ...
    if primary_target_len == 1 and not any('structural' == c['type'] for c in search_components):
        target_pattern = search_components[0]['pattern']
        for i, token in enumerate(tokens_lower):
            if target_pattern.fullmatch(token): all_target_positions.append(i)
    else:
        for i in range(len(tokens_lower) - primary_target_len + 1):
            match = True
            for k, component in enumerate(search_components):
    ...
    if freq == 0: return (pd.DataFrame(), 0, raw_target_input)

    collocate_directional_counts = Counter() 
    PUNCTUATION_COLLOCATES = PUNCTUATION
    ...
    for i in primary_target_positions:
        ...
        for j in range(start_index, end_index):
            ...
            w = tokens_lower[j]
            if w in PUNCTUATION_COLLOCATES or w.isdigit(): continue
    ...
    if collocate_regex:
        ...
        try: filtered_df = filtered_df[filtered_df['Collocate'].str.fullmatch(pattern, case=True, na=False)]
        except re.error: filtered_df = pd.DataFrame() 
    ...
    if collocate_pos_regex_input and not is_raw_mode:
        ...
        if full_pos_regex_list:
            ...
            try: filtered_df = filtered_df[filtered_df['POS'].str.contains(full_pos_regex, case=True, na=False, regex=True)]
            except re.error: filtered_df = pd.DataFrame()
    ...
    if collocate_lemma and 'Lemma' in filtered_df.columns and not is_raw_mode: 
        ...
        try: filtered_df = filtered_df[filtered_df['Lemma'].str.fullmatch(lemma_pattern, case=True, na=False)]
        except re.error: filtered_df = pd.DataFrame()
...
# ---------------------------
corpus_source = None
corpus_name = st.session_state.get('corpus_name', "No Corpus Loaded") # Use state name
df = st.session_state['processed_df'] # CRITICAL: Get DF from state
parallel_uploaded = st.session_state.get('parallel_mode', False)
...
with st.sidebar:
    ...
    if st.session_state.get('user_explicit_lang_code') == 'ID': initial_lang_index = 1
    elif st.session_state.get('user_explicit_lang_code') == 'OTHER': initial_lang_index = 2
    ...
    if corpus_mode == "Monolingual Corpus":
        ...
        selected_format_mono = st.selectbox(
            "1. Choose Format:",
            options=[
                ".txt (Raw Text/Linear)", ".xml (Raw Text/Linear)",
                ".txt verticalised (T/P/L columns)", ".xml verticalised (XML with <w> tags)",
                ".txt TreeTagger format", ".xml TreeTagger format"
            ],
    ...
    # --- B2. PARALLEL CORPUS UPLOAD ---
        ...
        if parallel_file_mode == "One corpus file":
            ...
            xml_src_file = st.file_uploader(
                ...
                type=["xml"], key="xml_src_file_upload", on_change=reset_analysis
            )
            xml_tgt_file = st.file_uploader(
                f"4. Upload Target Language XML ({tgt_lang_input})", 
                type=["xml"], key="xml_tgt_file_upload", on_change=reset_analysis
            )
            
    # --- D. LOAD BUTTON (CRITICAL FOR MANUAL EXECUTION) ---
    st.markdown("---")
    load_ready = (selected_corpus_name != "Select built-in corpus...") or \
                 (corpus_mode == "Monolingual Corpus" and st.session_state.get('mono_file_upload')) or \
                 (corpus_mode == "Parallel Corpus" and st.session_state.get('parallel_excel_file_upload')) or \
                 (corpus_mode == "Parallel Corpus" and st.session_state.get('xml_src_file_upload') and st.session_state.get('xml_tgt_file_upload'))

    
    if st.button("🚀 Load Corpus", key="manual_load_corpus", use_container_width=True, disabled=not load_ready, type="primary"):
        handle_corpus_loading()
        
    st.caption("Click 'Load Corpus' after selecting or uploading to make the tools available.")
    ...
    is_active_n = st.session_state['view'] == 'n_gram'
    ...
    if st.session_state['view'] != 'overview' and df_sidebar is not None:
        ...
        show_pos_tag = st.checkbox(
            "Show POS Tag", value=st.session_state.get('show_pos_tag', False), 
            key='show_pos_tag_checkbox', disabled=not has_pos_lemma_data
        )
        st.session_state['show_pos_tag'] = show_pos_tag

        show_lemma = st.checkbox(
            "Show Lemma", value=st.session_state.get('show_lemma', False), 
            key='show_lemma_checkbox', disabled=not has_pos_lemma_data
        )
        st.session_state['show_lemma'] = show_lemma
        
        if show_pos_tag or show_lemma: st.caption("Context displays in the format: **token/TAG{lemma}**")
        st.markdown("---")
        
        if st.session_state['view'] == 'concordance':
            st.subheader("Concordance Parameters")
            st.write("KWIC Context (Display)")
            kwic_left = st.number_input("Left Context (tokens)", min_value=1, max_value=20, value=st.session_state.get('kwic_left', 7), step=1, key="concordance_kwic_left")
            kwic_right = st.number_input("Right Context (tokens)", min_value=1, max_value=20, value=st.session_state.get('kwic_right', 7), step=1, key="concordance_kwic_right")
            ...
            pattern_search_window = st.number_input(
                "Search Window (tokens, each side)", min_value=1, max_value=10, value=st.session_state.get('pattern_search_window', 5), step=1, 
                key="pattern_search_window_input", on_change=trigger_analysis_callback 
            )
            
            pattern_collocate = st.text_input(
                "Collocate Word/Pattern (* for wildcard)", value=st.session_state.get('pattern_collocate_input', ''),
                key="pattern_collocate_input", on_change=trigger_analysis_callback 
            )
            
            if has_pos_lemma_data:
                pattern_collocate_pos_input = st.text_input(
                    "Collocate POS Tag Pattern (Wildcard/Concatenation)", value=st.session_state.get('pattern_collocate_pos_input', ''),
                    key="pattern_collocate_pos_input", help="E.g., V* (Verbs), *G (Gerunds), NNS|NNP (Plural/Proper Nouns). Filters collocates by POS tag.",
            ...
            n_gram_size = st.slider(
                "N-Gram Size (N)", min_value=1, max_value=5, value=st.session_state.get('n_gram_size', 2), step=1, 
                key="n_gram_size_slider", on_change=trigger_n_gram_analysis_callback
            )
            ...
            for i in range(1, n_gram_size + 1):
                ...
                filter_input = st.text_input(
                    f"Position {i} Filter", value=default_val, key=input_key,
                    on_change=trigger_n_gram_analysis_callback, args=(), help=help_text
                )
            ...
            max_collocates = st.number_input("Max Collocates to Show (Network/Tables)", min_value=5, max_value=100, value=st.session_state.get('max_collocates', 20), step=5, key="coll_max_collocates")
            coll_window = st.number_input("Collocation window (tokens each side)", min_value=1, max_value=10, value=st.session_state.get('coll_window', 5), step=1, key="coll_window_input")
            ...
            if has_pos_lemma_data:
                collocate_pos_regex_input = st.text_input(
                    "Filter by POS Tag Pattern (Wildcard/Concatenation)", value=st.session_state.get('collocate_pos_regex_input_coll', ''), 
                    key="collocate_pos_regex_input_coll_tag", help="E.g., V* (Verbs), NNS|NNP (Plural/Proper Nouns)."
                )
                ...
                if all_pos_tags:
                    selected_pos_tags = st.multiselect(
                        "OR Filter by specific POS Tag(s)", options=all_pos_tags,
                        default=st.session_state.get('selected_pos_tags_input', None), key="selected_pos_tags_input",
            ...
            if has_pos_lemma_data and 'lemma' in df_sidebar.columns:
    ...
    if explicit_lang_code != 'EN' and CEFR_FEATURE_AVAILABLE: cefr_status += f" (Disabled for **{explicit_lang_code}** corpus)"
    st.markdown(cefr_status)

    ipa_status = f"**IPA Transcription:** {'✅ ACTIVE' if IPA_FEATURE_AVAILABLE else '❌ DISABLED (Install `eng-to-ipa`)'}"
    if explicit_lang_code != 'EN' and IPA_FEATURE_AVAILABLE: ipa_status += f" (Disabled for non-English corpus)"
...
if st.session_state['view'] == 'overview':
    ...
    with col1:
        ...
        if not freq_df.empty:
            if not WORDCLOUD_FEATURE_AVAILABLE:
                st.info("⚠️ **Word Cloud Feature Disabled:** Visualization requires the external `wordcloud` library.")
            else:
    ...
    with col2:
        ...
        if not freq_df.empty:
            ...
            if 'pos' in freq_head.columns: display_cols.insert(1, 'pos')
        ...
        else: st.info("Frequency data not available.")
...
if st.session_state['view'] != 'overview' and st.session_state['view'] != 'dictionary' and st.session_state['view'] != 'n_gram':
    ...
    st.session_state['analyze_btn'] = analyze_btn
    st.session_state['trigger_analyze'] = False
...
if st.session_state['view'] == 'n_gram':
    ...
    if manual_analyze_btn: analyze_n_gram = True
    
    if analyze_n_gram:
        with st.spinner(f"Generating and filtering {st.session_state['n_gram_size']}-grams..."):
            n_gram_df = generate_n_grams(
                df, st.session_state['n_gram_size'], st.session_state['n_gram_filters'], is_raw_mode,
                corpus_name
            )
    ...
    st.download_button(
        download_label, data=df_to_excel_bytes(n_gram_df), 
...
if st.session_state['view'] == 'concordance' and st.session_state.get('analyze_btn', False) and st.session_state.get('typed_target_input'):
    ...
    with st.spinner("Searching corpus and generating concordance..."):
        kwic_rows, total_matches, raw_target_input, literal_freq, sent_ids, breakdown_df = generate_kwic(
            ...
            show_pos=show_pos_tag, show_lemma=show_lemma
        )
    ...
    if not breakdown_df.empty:
        ...
        st.download_button(
            "⬇ Download full token breakdown (xlsx)", data=df_to_excel_bytes(breakdown_df), 
...
if st.session_state['view'] == 'dictionary':
    ...
    if forms_list.empty and word_freq > 0:
        forms_list = pd.DataFrame([{'token': current_dict_word, 'pos': '##', 'lemma': '##'}])
    ...
    if ipa_active:
        ...
        except Exception: ipa_active = False 
    if not ipa_active: forms_list.insert(forms_list.shape[1], 'IPA Transcription', 'NA')
    ...
    if not IPA_FEATURE_AVAILABLE and is_english_corpus: st.info("💡 **Phonetic Transcription (IPA) feature requires the `eng-to-ipa` library.**")
    elif not is_english_corpus and IPA_FEATURE_AVAILABLE: st.info(f"💡 Phonetic Transcription (IPA) is disabled for non-English corpus ({SOURCE_LANG_CODE}).")
        
    if not CEFR_FEATURE_AVAILABLE and is_english_corpus: st.info("💡 **CEFR Categorization feature requires the `cefrpy` library.**")
    ...
    if related_regex_forms:
        ...
        st.text_area(
            "Related Forms (by regex)", ", ".join(related_regex_forms), height=100, 
            key=f"regex_forms_output_{current_dict_word}" 
        )
    else: st.info(f"No related tokens found matching the regex pattern.")
    ...
    with st.spinner(f"Fetching random concordance examples for '{current_dict_word}'..."):
        kwic_rows, total_matches, _, _, sent_ids, _ = generate_kwic(
            df, current_dict_word, kwic_left, kwic_right, random_sample=True, limit=KWIC_MAX_DISPLAY_LINES,
            is_parallel_mode=is_parallel_mode, show_pos=show_pos_tag, show_lemma=show_lemma
        )
    ...
    if kwic_rows:
        ...
        st.markdown(scrollable_html, unsafe_allow_html=True)
    else: st.info("No examples found.")
    ...
    if stats_df_sorted.empty:
        ...
        display_collocation_kwic_examples(
            df_corpus=df, node_word=current_dict_word, top_collocates_df=top_collocates, 
            window=coll_window, limit_per_collocate=1, is_parallel_mode=is_parallel_mode,
            target_sent_map=st.session_state['target_sent_map'], show_pos=show_pos_tag,
...
if st.session_state['view'] == 'collocation' and st.session_state.get('analyze_btn', False) and st.session_state.get('typed_target_input'):
    ...
    if st.button("🧠 Interpret Collocation Results (LLM)", key="llm_collocation_btn"):
        interpret_results_llm(
            target_word=raw_target_input, analysis_type="Collocation",
    ...
    if PYVIS_FEATURE_AVAILABLE:
        with col_left_graph:
            ...
            else: st.info("No Left-dominant collocates found.")
        ...
        with col_right_graph:
            ...
            else: st.info("No Right-dominant collocates found.")
    else:
    ...
    scroll_style = f"""<style> .scrollable-table {{ max-height: 400px; overflow-y: auto; }} </style>"""
    st.markdown(scroll_style, unsafe_allow_html=True)
    ...
    st.download_button(
        f"⬇ Download full LL results (xlsx)", data=df_to_excel_bytes(stats_df_sorted), 
        file_name=f"{primary_target_mwu.replace(' ', '_')}_LL_full_filtered.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        f"⬇ Download full MI results (obs≥{mi_min_freq}) (xlsx)", data=df_to_excel_bytes(full_mi_all), 
        file_name=f"{primary_target_mwu.replace(' ', '_')}_MI_full_obsge{mi_min_freq}_filtered.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    ...
    display_collocation_kwic_examples(
        df_corpus=df, node_word=primary_target_mwu, top_collocates_df=full_ll, 
        window=coll_window, limit_per_collocate=1, is_parallel_mode=is_parallel_mode,
        target_sent_map=st.session_state['target_sent_map'], show_pos=show_pos_tag,
        show_lemma=show_lemma
    )
    ...
    display_collocation_kwic_examples(
        df_corpus=df, node_word=primary_target_mwu, top_collocates_df=full_mi, 
        window=coll_window, limit_per_collocate=1, is_parallel_mode=is_parallel_mode,
        target_sent_map=st.session_state['target_sent_map'], show_pos=show_pos_tag,
        show_lemma=show_lemma
    )
